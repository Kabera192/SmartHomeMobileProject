...
import 'package:new_sensors/components/constants.dart';
...
class NewMapPage extends StatefulWidget {
  ...
  @override
  State<NewMapPage> createState() => _MapPageState();
}

class _MapPageState extends State<NewMapPage> {
      ...
      body: _currentP == null
          ? const Center(
              child: CircularProgressIndicator(
                color: Color.fromARGB(255, 5, 56, 209),
              ),
            )
          : GoogleMap(
  ...
  void _triggerOutSideNotification() async {
    if (!_notificationSentOutSide) {
      ...
      await flutterLocalNotificationsPlugin.show(
        0,
        'MondayFence',
        'You are now outside the GeoFence',
        platformChannelSpecifics,
      );
    ...
    setState(() {
      ...
      if (polylines.containsKey(PolylineId("path"))) {
        ...
        polylines[PolylineId("path")] = Polyline(
          ...
          color: Color.fromARGB(255, 1, 38, 173),
          points: [newLocation],
          width: 5,
        );
      }
